...
export const enabled=Boolean(config.apiKey&&config.projectId)
const app=enabled?initializeApp(config):null
export const db=app?getFirestore(app):null
const auth=app?getAuth(app):null
export async function login(){if(!auth)return null;if(!auth.currentUser)await signInAnonymously(auth);return auth.currentUser}
