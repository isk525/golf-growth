...
const getClubInsight=club=>{
  ...
  const tipLogs=clubLogs.filter(
    log=>
      log.recordType==='tip' ||
      (!log.recordType && log.selectedTip)
  );

  const successRate=shotLogs.length
    ?Math.round(
      shotLogs.reduce(
        (sum,log)=>
          sum+(shotPoint[log.shotOutcome]??0),
        0
      )/shotLogs.length*100
    )
    :null;
  ...
  shotLogs.forEach(log=>{
    (log.misses||[]).forEach(miss=>{
      missCounts[miss]=(missCounts[miss]||0)+1;
    });
  });
  ...
  tipLogs.forEach(log=>{
    ...
    if(!tipMap[log.selectedTip]){
      tipMap[log.selectedTip]={
        total:0,
        count:0
      };
    }

    tipMap[log.selectedTip].total+=
      resultPoint[log.result]??0;

    tipMap[log.selectedTip].count+=1;
  });

  const bestTip=Object.entries(tipMap)
    .map(([name,value])=>({
      name,
      count:value.count,
      rate:Math.round(
...
const getBestFocusForClub=(club,swing)=>{
  ...
  rows.forEach(log=>{
    if(!groups[log.selectedTip]){
      groups[log.selectedTip]={
        total:0,
        count:0
      };
    }

    groups[log.selectedTip].total+=
      resultPoint[log.result]??0;

    groups[log.selectedTip].count+=1;
  });
...
const recs=useMemo(()=>{
  ...
  return d.distances
    .map(item=>{
      ...
      const focus=getBestFocusForClub(
        item.club,
        item.swing
      );

      const distanceDiff=
        Math.abs(item.yards-target);

      const statisticalBonus=
        stat.rate===null
          ?0
          :stat.rate/18;

      const penalty=
        (swingPenalty[item.swing]||0)
        +
        longIronShortPenalty(
          item.club,
          target
        );
...
const suggestionContextMatch=(log,p)=>{if(log.club!==p.club)return false;if(p.club==='パター'){if(log.puttIssue!==p.puttIssue||log.puttSlope!==p.puttSlope)return false;if(p.puttIssue==='距離感')return Math.abs((+log.puttMeters||0)-estimatedPuttMeters())<=1&&log.puttDistanceResult===p.puttDistanceResult;return log.puttDirection===p.puttDirection;}return(log.swing||'フル')===(p.swing||'フル')&&log.misses?.some(m=>p.misses.includes(m))};
    ...
    {activeClubInsight.count===0?(
      ...
      <>
        <div className="themeStats">
          <span>
            記録
            <b>{activeClubInsight.count}件</b>
          </span>

          <span>
            精度
            <b>
              {activeClubInsight.successRate}%
            </b>
          </span>
        </div>

        {activeClubInsight.topMisses.length>0&&(
          <p>
            ミス傾向：
            {activeClubInsight.topMisses
              .map(x=>x.name)
              .join('・')}
          </p>
        )}
        ...
        {activeClubInsight.bestTip&&(
          <div className="themeFocus">
            <small>ここを意識</small>
            <b>
              {activeClubInsight.bestTip.name}
            </b>
            <span>
              過去効果
              {activeClubInsight.bestTip.rate}%
              ・
              {activeClubInsight.bestTip.count}回
            </span>
          </div>
        )}
      </>
...
<Field n="今回のショット">
  ...
  <div className="shotOutcomeChoices">
    {SHOT_RESULTS.map(([value,label])=>(
      <Choice
        key={value}
        on={practice.shotOutcome===value}
        f={()=>setPractice({
          ...practice,
          shotOutcome:value,
          misses:
...
{(
  ...
  <Field n="どのようなミスだったか">
    <small className="fieldHelp required">
      方向とは別に、曲がり方や
      当たり方を1つ以上選択
    </small>
    ...
    <div className="misses">
      {MISS.map(([n,desc])=>(
        <Choice
          key={n}
          on={practice.misses.includes(n)}
          f={()=>setPractice({
            ...practice,
            misses:practice.misses.includes(n)
              ?practice.misses.filter(x=>x!==n)
              :[...practice.misses,n]
          })}
        >
          <b>{n}</b>
...
{(
  ...
  const selected=
    practice.selectedTips.includes(x.name);
  ...
  return(
    <button
      ...
      onClick={()=>{
        const next=selected
          ?practice.selectedTips.filter(
            name=>name!==x.name
          )
          :[
            ...practice.selectedTips,
            x.name
          ];
    ...
    >
      <div>
        ...
        <p>
          {tipHelp[x.name]||
            'この項目を1つだけ試し、打球の変化を確認します。'}
        </p>

        <small className="tipEvidence">
          {x.trials===0
            ?'未試行'
            :`過去効果 ${x.rate}%・${x.trials}回${
              x.latest
                ?`・直近 ${x.latest}`
                :''
            }`
...
<Primary
  f={()=>{
    if(
      practice.selectedTips.length===0
    ){
      return toast(
        '試す改善策を1つ以上選択してください'
      );
    ...
    const initialResults=
      Object.fromEntries(
        practice.selectedTips.map(
          name=>[
            name,
            practice.tipResults[name]||'○'
          ]
        )
      );
...
>
  選んだ改善策を試す
  （{practice.selectedTips.length}件）
</Primary>

{excludedSuggestions.length>0&&<details className="excludedTips"><summary>今回は除外した改善 {excludedSuggestions.length}件</summary>{excludedSuggestions.map(x=><button key={x.name} onClick={()=>{setPractice({...practice,selectedTip:x.name});setPracticeView('result')}}><span><b>{x.name}</b><small>同じ条件で過去に悪化（×）</small></span><em>もう一度試す</em></button>)}</details>}</>}

{practiceView==='result'&&<><Back f={()=>setPracticeView('suggestions')}/>
<Card>
<small>試した改善策ごとの結果</small>

<div className="multiTipResults">
  {practice.selectedTips.map(name=>(
    <section
      className="tipResultItem"
      key={name}
    >
      <div>
        <b>{name}</b>
...
{recs[0]&&<div className="bestClub"><strong>{recs[0].club}</strong><span>{recs[0].swing}・{recs[0].yards}y</span><small>{recs[0].rate===null?'実績なし':`効果 ${recs[0].rate}%`}</small></div>}<button className="assistToggle" disabled={recs.length<2} onClick={()=>setAssistOpen(v=>!v)}>{assistOpen?'閉じる':`候補${recs.length}件`}</button></div>
{recs[0]?.focus&&(
  <div className="roundFocus">
    <span>
      <Sparkles/>
      意識ポイント
    </span>

    <b>{recs[0].focus.name}</b>

    <small>
      過去効果
      {recs[0].focus.rate}%
      ・
      {recs[0].focus.count}回
    </small>
  </div>
)}
{assistOpen&&<div className="assistMore">{recs.slice(1).map(x=><div key={x.club+x.swing}><b>{x.club}</b><span>{x.swing}・{x.yards}y</span><small>{x.rate===null?'実績なし':`効果 ${x.rate}%・${x.count}回`}</small></div>)}</div>}</section><button className="cancelRound" onClick={async()=>{if(confirm('このラウンドを中止しますか？ 入力中のスコアは履歴に残りません。')){const cancelled={...d.round,status:'cancelled',cancelledAt:new Date().toISOString()};if(enabled){await login();await setDoc(doc(db,'rounds',cancelled.id),cancelled)}setD(x=>({...x,round:null}));setHole(0);setPage('home');toast('ラウンドを中止しました')}}}>ラウンドを中止</button><div className="hole"><button disabled={!hole} onClick={()=>setHole(h=>h-1)}><ChevronLeft/></button><h2>H{d.round.holes[hole].no}<small>Par {d.round.holes[hole].par}</small></h2><button disabled={hole===d.round.holes.length-1} onClick={()=>setHole(h=>h+1)}><ChevronRight/></button></div>{d.round.players.map(p=>{const h=d.round.holes[hole];return <Card key={p}><div className="row"><b>{p}</b><strong>{h.scores[p]||'-'}</strong></div><div className="scores">{S.map(n=><Choice key={n} on={h.scores[p]===n} f={()=>sync({...d.round,holes:d.round.holes.map((x,i)=>i===hole?{...x,scores:{...x.scores,[p]:n}}:x)})}>{n}</Choice>)}</div></Card>})}<Primary f={()=>hole<d.round.holes.length-1?setHole(h=>h+1):finish()}>{hole<d.round.holes.length-1?'次のホール':'結果を保存'}</Primary></div>}
